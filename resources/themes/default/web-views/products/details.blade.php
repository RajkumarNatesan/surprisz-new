                            ...
                            <div class="__review-overview card card-body h-100">
                                <ul class="nav nav-tabs nav--tabs d-flex justify-content-center gap-2" role="tablist">
                                    ...
                                    @if ($product->highlights)
                                        <li class="nav-item">
                                            <a class="nav-link __inline-27" href="#highlights" data-toggle="tab"
                                                role="tab">
                                    ...
                                    @if ($product->faqs)
                                        <li class="nav-item">
                                            <a class="nav-link __inline-27" href="#faq" data-toggle="tab"
                                                role="tab">
                                                {{ translate('FAQ') }}
                                            </a>
                                        </li>
                                    @endif
                                    @if (count($product->addOnGroups) > 0)
                                        <li class="nav-item">
                                            <a class="nav-link __inline-27" href="#addons" data-toggle="tab"
                                                role="tab">
                                                {{ translate('addons') }}
                                            </a>
                                        </li>
                                ...
                                <div class="tab-content px-lg-3">
                                        ...
                                        <div class="details-content-wrap ov-hidden show-more--content">
                                            ...
                                            @if (
                                                !$product['details'] &&
                                                    !$product['short_description'] &&
                                                    !$product['highlights'] &&
                                                    ($product->video_url == null || !str_contains($product->video_url, 'youtube.com/embed/')))
                                                <div>
                                    ...
                                    @if ($product->highlights)
                                        <div class="tab-pane fade text-justify" id="highlights" role="tabpanel">
                                            <div class="row pt-2 specification">
                                                <div class="text-body col-lg-12 col-md-12 fs-13 details-text-justify">
                                                    {!! $product->highlights !!}
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    @if ($product->faqs)
                                        <div class="tab-pane fade text-justify" id="faq" role="tabpanel">
                                            <div class="row pt-2 specification">
                                                <div class="text-body col-lg-12 col-md-12 fs-13 details-text-justify">
                                                    {!! $product->faqs !!}
                                                </div>
                                    ...
                                    @if (count($product->addOnGroups) > 0)
                                        <div class="tab-pane fade text-justify" id="addons" role="tabpanel">
                                            @php($currency_symbol = '₹')
                                            @php($currency_position = getWebConfig(name: 'currency_symbol_position'))
                                            <div id="addon-currency-info" class="d-none"
                                                data-symbol="{{ $currency_symbol }}"
                                                data-position="{{ $currency_position }}"
                                                data-decimal="{{ getWebConfig(name: 'decimal_point_settings') ?? 2 }}">
                                            </div>
                                            <div class="row pt-2 specification">
                                                <div class="col-lg-12 col-md-12">
                                                            ...
                                                            <div class="row g-3">
                                                                @foreach ($group->items as $item)
                                                                    <div class="col-12 mb-2">
                                                                        <div class="card border-aliceblue shadow-sm addon-card">
                                                                            <div class="card-body p-2 p-sm-3">
                                                                                <div class="d-flex flex-wrap flex-sm-nowrap align-items-center justify-content-between gap-3">
                                                                                    <div class="d-flex align-items-center flex-grow-1 min-w-0">
                                                                                        <div class="form-check custom-control custom-checkbox px-0 flex-shrink-0">
                                                                                            <input class="addon-checkbox"
                                                                                                type="checkbox"
                                                                                                name="addon_item_ids[]"
                                                                                                value="{{ $item->id }}"
                                                                                        ...
                                                                                        <label class="form-check-label font-weight-bold text-dark ml-2 mb-0 cursor-pointer text-truncate"
                                                                                            for="addon-item-{{ $item->id }}"
                                                                                            style="font-size: 14px;">
                                                                                            {{ $item->name }}
                                                                                        </label>
                                                                                    </div>
                                                                                    <div class="d-flex align-items-center gap-3 flex-shrink-0">
                                                                                        <span class="text-primary font-weight-bold fs-14">
                                                                                            {{ webCurrencyConverter(amount: $item->price) }}
                                                                                        </span>
                                                                                        <div class="d-flex align-items-center border rounded bg-white">
                                                                                            <button type="button"
                                                                                                class="btn btn-sm px-2 addon-qty-minus"
                                                                                                data-target="addon-qty-{{ $item->id }}"
                                                                                                style="background: #f8f9fa;">-</button>
                                                                                            <input type="number"
                                                                                                name="addon_quantities[{{ $item->id }}]"
                                                                                                id="addon-qty-{{ $item->id }}"
                                                                                                class="form-control form-control-sm text-center border-0 addon-qty-input"
                                                                                                value="1" min="1"
                                                                                                readonly
                                                                                                style="width: 40px; background: transparent;"
                                                                                                form="add-to-cart-form">
                                                                                            <button type="button"
                                                                                                class="btn btn-sm px-2 addon-qty-plus"
                                                                                                data-target="addon-qty-{{ $item->id }}"
                                                                                                style="background: #f8f9fa;">+</button>
                                                                                        </div>
                                                                                    </div>
                                                    ...
                                                    <div
                                                        class="mt-4 p-3 bg-aliceblue rounded d-flex justify-content-between align-items-center border">
                                                        <h5 class="mb-0 font-weight-bold">
                                                            {{ translate('total_addons_price') }}:</h5>
                                                        <h5 class="mb-0 font-weight-bold text-primary">
                                                            <span
                                                                id="addons-total-price-display">{{ webCurrencyConverter(amount: 0) }}</span>
                                                        </h5>
                                                    </div>
                                                </div>
                                            </div>
    ...
    <script>
        $(document).ready(function() {
            function formatCurrency(amount) {
                let currencyInfo = $('#addon-currency-info');
                let symbol = currencyInfo.data('symbol');
                let position = currencyInfo.data('position');
                let decimalPoints = parseInt(currencyInfo.data('decimal'));
                let formattedAmount = amount.toFixed(decimalPoints).replace(/\d(?=(\d{3})+\.)/g, '$&,');
            ...
            function updateAddonsTotal() {
                ...
                $('.addon-checkbox:checked').each(function() {
                    ...
                    let qtyInput = $('#addon-qty-' + itemId);
                    let qty = parseInt(qtyInput.val()) || 1;
                    total += price * qty;
                });

                $('#addons-total-price-display').text(formatCurrency(total));
            }

            $(document).on('change', '.addon-checkbox, .addon-qty-input', function() {
                updateAddonsTotal();
            });

            $(document).on('click', '.addon-qty-plus', function() {
                let targetId = $(this).data('target');
                let input = $('#' + targetId);
                input.val(parseInt(input.val()) + 1).trigger('change');
            });

            $(document).on('click', '.addon-qty-minus', function() {
                let targetId = $(this).data('target');
                let input = $('#' + targetId);
